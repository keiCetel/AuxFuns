function H = mseb_ak(x,y,errBar,lineProps,transparent)
...
% Technical University of Denmark, Cognitive systems - September 2014
%
% Inputs
...
%              fields.
%       .col - cell array defining the color of each line, e.g., 'b' for
%              blue.
%            - AK change: array can contain number triplets, e.g. 
%              [.5 .7 .2], instead of strings
...
%              other lines. 
%  .patchsat - AK change: transparency of error patches, between 0 and 1;
%              default: .25
%             
...
for c = C:-1:1
    ...
    if ~isfield(lineProps,'patchsat'),	lineProps.patchsat = .25; end    
%     patchSaturation=0.15; %How de-saturated or transparent to make the patch
    patchSaturation=lineProps.patchsat; %How de-saturated or transparent to make the patch
...
for c = 1:C
    ...
    %and put it back (yuk!)
    delete(H(c).mainLine)
%     lp = [lineProps.style lineProps.col{c}];
    H(c).mainLine=plot(x(c,:),y(c,:),lineProps.style,'Color',lineProps.col{c},'linewidth',lineProps.width);
